...
class BasicRNN(BaseModel):
    ...
    def __init__(self, config):
        super(BasicRNN, self).__init__(config)
    
    def create_model(self):
        
        self.RNN = nn.RNN(input_size=self.pose_size, 
        ...
        self.dense = nn.Linear(self.config.hidden_dim, self.pose_size)
    ...
    def forward(self, batch: AMASSBatch):
        ...
        model_in = batch.poses[:, :self.config.seed_seq_len]
        output, hn = self.RNN(model_in)
        prediction = self.dense(output[:,-1,:]).unsqueeze(1)

        predictions.append(prediction)

        # Now we have the choice of either using the predicted values or the ground truth
        for i in range(self.config.target_seq_len-1):
            output, hn = self.RNN(prediction,hn)
            prediction = self.dense(output)
            predictions.append(prediction)
        
        stacked_output = torch.stack(predictions, dim=1).squeeze(2)

        model_out['predictions'] = stacked_output
        return model_out
...
class LSTMModel(BasicRNN):
    ...
    def create_model(self):
        ...
        self.lstm = nn.LSTM(self.pose_size,
    ...
    def forward(self, batch: AMASSBatch):
        ...
        model_in = batch.poses[:, :self.config.seed_seq_len]
        output, state = self.lstm(model_in)
        prediction = self.dense(output[:,-1,:]).unsqueeze(1)
        ...
        # Now we have the choice of either using the predicted values or the ground truth
        for i in range(self.config.target_seq_len-1):
            output, state = self.lstm(prediction,state)
            prediction = self.dense(output)
            predictions.append(prediction)
